...
export class CartService {
  ...
  private cartUrl = `$"http://main.ap-south-1.elasticbeanstalk.com/api/v1/cart`;
  localMap = {};
  ...
  getCart(): Observable<ProductInOrder[]> {
    ...
    if (this.user.email) {
      if (localCart.length > 0) {
        return this.http.post<Cart>("http://main.ap-south-1.elasticbeanstalk.com/api/v1/cart/merge/"+this.user.email, localCart).pipe(
          tap(() => {
            this.clearLocalCart();
          }),
        ...
        return this.http.get<Cart>("http://main.ap-south-1.elasticbeanstalk.com/api/v1/cart/get/"+this.user.email).pipe(
          map((cart) => cart.products),
          catchError(() => of([]))
        );
      }
  ...
  addItem(productInOrder): Observable<boolean> {
    ...
    if (!this.user.email) {
      ...
      return this.http.post<boolean>("http://main.ap-south-1.elasticbeanstalk.com/api/v1/cart/add/"+this.user.email, {
  ...
  update(productInOrder): Observable<ProductInOrder> {
    ...
    if (this.user.email) {
     ...
     console.log("update called");
      return this.http.put<ProductInOrder>("http://main.ap-south-1.elasticbeanstalk.com/api/v1/cart/modify/"+productInOrder.productId+"/"+this.user.email, productInOrder.count);
    }
    else{
      return null;
    }
  ...
  remove(productInOrder) {
    ...
    if (!this.user.email) {
      ...
      return this.http.delete("http://main.ap-south-1.elasticbeanstalk.com/api/v1/cart/delete/"+productInOrder.productId+"/"+this.user.email);
    }
  }
