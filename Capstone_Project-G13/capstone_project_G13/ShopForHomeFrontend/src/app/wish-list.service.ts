...
export class WishListService {
  ...
  getPage(page = 1, size = 10): Observable<any> {
    return this.http.get("http://main.ap-south-1.elasticbeanstalk.com/api/v1/wishlist/list/"+this.user.email+"?page="+page+"&size="+size);
  }
  ...
  addToWishList(productId): Observable<WishList> {
    ...
    if (this.user.email) {
      if (productId) {
        return this.http.post<WishList>("http://main.ap-south-1.elasticbeanstalk.com/api/v1/wishlist/add/"+productId+"/"+this.user.email, null);
      }
    }
    return null;
  }
  ...
  removeFromWishList(productId: string): Observable<Boolean> {
    ...
    if (this.user.email) {
      ...
      return this.http.delete<Boolean>("http://main.ap-south-1.elasticbeanstalk.com/api/v1/wishlist/delete/"+productId+"/"+this.user.email);
    } else {
      console.log("hi");
      return of(false);
    }
